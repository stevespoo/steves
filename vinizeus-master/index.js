...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Senta e espera, tô fazendo!⏳',
				success: '[ ✔ ]Funciona',
				...
				error: {
					stick: '[❌ ] Deu certo não viado, ocorreu um erro ao converter a imagem em figurinha',
					Iv: '[ ❌ ]Link inválido.'
				},
				only: {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '554399963203@s.whatsapp.net'//Nomer Owner
                        const isEventon = isGroup ? event.includes(from) : false
                        const isRegister = checkRegisteredUser(sender)
                        pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefined

                        // Options
                        const botName = 'Steves'//Nama Bot
                        const ownerName = 'steves<3'//Nama Owner
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
				if (args.length < 1) return reply("Onde está a url fdp?")
               ...
               if (data.error) return reply(data.error)
				...
				case 'tm':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*[ STEVES BROADCAST ]*\n\n${body.slice(4)}`)
						}
	    ...
	    case 'kapankah':
            ...
            break
				...
				case 'audio':
					...
					break
                                case 'clearall':
					if (!isOwner) return reply('Quem é voce fi?')
					anu = await client.chats.all()
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Cade o texto filha da puta?')
					teks = body.slice(5)
